...
The GitHub workflow [build-docker-opencode.yaml](.github/workflows/build-docker-opencode.yaml) and [Dockerfile](Dockerfile) of this repository continously build a full-stack container image based 
on Debian (currently Trixie version)to run [Opencode](https://opencode.ai/), aka *"the Open Source coding agent"*.  Running opencode in Docker doesn't restrict 
its function. It matches the experience of a native install: you can still run the agent in advanced terminals like [Ghostty](https://ghostty.org/docs/about) 
or [Alacritty](https://github.com/alacritty/alacritty) to enjoy the optimal User Experience (UX).

Give it try: in your Ghostty (my personal preference) / Alacritty / regular terminal, just execute 
...
Opencode will answer you who he is. The model `opencode/big-pickle` is a free model offered by Opencode to facilitate 
initial Opencode discovery.
...
You can see previous executions of this workflow in [Actions tab](https://github.com/didier-durand/opencode-vibe/actions/workflows/build-docker-opencode.yaml) 
of this repository.
